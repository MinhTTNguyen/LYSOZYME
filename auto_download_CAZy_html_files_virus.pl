...
my $path="/home/mnguyen/Research/Lysozyme/GH25/CAZy";
my $filein_cazy_list="CAZy_list.txt";
my $folderout="HTML_virus";
...
foreach my $cazy_family (@cazy_list)
{
	...
	my $url_all='http://www.cazy.org/'.$cazy_family.'_viruses.html';
	...
	while (<First_table>)
	{
		...
		if ($_=~/href\=\'$cazy_family\_viruses\.html\?debut\_TAXO\=(\d+)\#pagination\_TAXO\'/)
		{
			my $temp=$1;
	...
	if ($number)
	{
		for (my $i=100; $i<=$number; $i=$i+100)
		{
			...
			my $url='http://www.cazy.org/'.$cazy_family.'_viruses.html?debut_TAXO='.$i.'#pagination_TAXO';
